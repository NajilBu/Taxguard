...
if (!is_array($payload) || !in_array($payload['action'] ?? '', ['load','save','forms','login','users:list','users:save','users:delete','company:profile:get','company:profile:save','clients:fields:rename','documents:list','documents:save','documents:get','documents:delete','data:export','data:export-xlsx','data:read-xlsx','data:preview-import','data:import','clients:import-csv','clients:import-xlsx','clients:fields:get','clients:fields:save','calendar:list','calendar:save','calendar:delete'], true)) failRequest(400, 'Invalid database operation.');
