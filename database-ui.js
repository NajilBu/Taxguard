...
function clientSelectionRow(client,inputName,searchAttribute,checked){
  const pulled=client.status==='Pulled out';
  return `<tr class="client-selection-${pulled?'pulled':'active'}" ${searchAttribute}="${esc(`${client.name} ${client.tin||''}`.toLowerCase())}"><td><input type="checkbox" name="${inputName}" value="${client.id}" ${checked?'checked':''}></td><td>${esc(client.name)}</td><td>${esc(client.tin||'')}</td><td><span class="client-selection-status">${pulled?'Pulled out':'Active'}</span></td></tr>`;
}
function openReportSetup(reportType){
  ...
  dialog.innerHTML=`<h2>Set up ${esc(titles[reportType]||titles.summary)}</h2><p>Choose the tax years and clients to include. The preview, Excel export, and PDF will use this selection.</p><form id="report-setup-form"><div class="data-transfer-year-range"><label>From year<input id="report-year-from" type="number" min="2000" max="2100" value="${year}" required></label><label>To year<input id="report-year-to" type="number" min="2000" max="2100" value="${year}" required></label></div><div class="data-client-toolbar"><input id="report-client-search" type="search" placeholder="Search client or TIN" aria-label="Search clients"></div><p id="report-client-count" class="subtle"></p><div class="data-transfer-client-table"><table><thead><tr><th><input type="checkbox" id="report-select-all" aria-label="Select or clear all visible clients" checked></th><th>Client</th><th>TIN</th><th>Status</th></tr></thead><tbody>${ordered.map(c=>clientSelectionRow(c,'report-client','data-report-client-search',true)).join('')}</tbody></table></div><p id="report-setup-error" role="alert"></p><div class="modal-actions"><button type="button" class="btn" id="cancel-report-setup">Cancel</button><button type="submit" class="btn primary">Open report preview</button></div></form>`;
  const rows=[...dialog.querySelectorAll('[data-report-client-search]')],count=dialog.querySelector('#report-client-count');
  const selectAll=dialog.querySelector('#report-select-all');
  const updateCount=()=>{count.textContent=`${dialog.querySelectorAll('[name="report-client"]:checked').length} of ${ordered.length} clients selected`;const visible=rows.filter(row=>!row.hidden),selected=visible.filter(row=>row.querySelector('input').checked).length;selectAll.checked=visible.length>0&&selected===visible.length;selectAll.indeterminate=selected>0&&selected<visible.length;};
  dialog.querySelector('#report-client-search').oninput=e=>{const query=e.target.value.trim().toLowerCase();rows.forEach(row=>row.hidden=!!query&&!row.dataset.reportClientSearch.includes(query));updateCount();};
  selectAll.onchange=()=>{rows.filter(row=>!row.hidden).forEach(row=>row.querySelector('input').checked=selectAll.checked);updateCount();};
...
function openDataExportDialog(){
  ...
  dialog.innerHTML=`<h2>Export client data</h2><p>Select clients and a year range. Each selected client includes their profile and filing records for the selected years.</p><div class="data-client-toolbar"><input id="export-client-search" type="search" placeholder="Search by client name or TIN"></div><form id="data-export-form"><div class="data-transfer-client-table"><table><thead><tr><th><input type="checkbox" id="select-all-export-clients" aria-label="Select or clear all visible clients"></th><th>Client</th><th>TIN</th><th>Status</th></tr></thead><tbody>${clients.map(c=>clientSelectionRow(c,'export-client','data-client-search',false)).join('')}</tbody></table></div><div class="data-transfer-year-range"><label>From year<input type="number" id="export-year-from" min="2000" max="2100" value="${currentYear}"></label><label>To year<input type="number" id="export-year-to" min="2000" max="2100" value="${currentYear}"></label></div><div class="modal-actions"><button type="button" class="btn" id="cancel-data-export">Cancel</button><button class="btn primary">Export Excel workbook</button></div></form>`;
  const rows=[...dialog.querySelectorAll('[data-client-search]')];
  const selectAll=dialog.querySelector('#select-all-export-clients');
  const updateSelection=()=>{const visible=rows.filter(row=>!row.hidden),selected=visible.filter(row=>row.querySelector('input').checked).length;selectAll.checked=visible.length>0&&selected===visible.length;selectAll.indeterminate=selected>0&&selected<visible.length;};
  dialog.querySelector('#export-client-search').oninput=e=>{const query=e.target.value.trim().toLowerCase();rows.forEach(row=>row.hidden=!!query&&!row.dataset.clientSearch.includes(query));updateSelection();};
  selectAll.onchange=()=>{rows.filter(row=>!row.hidden).forEach(row=>row.querySelector('input').checked=selectAll.checked);updateSelection();};
  dialog.querySelector('.data-transfer-client-table').onchange=updateSelection;
