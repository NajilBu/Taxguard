...
function render(){ensureAutomaticYear();document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('selected',b.dataset.page===page));const breadcrumb=document.querySelector('.breadcrumb');breadcrumb.innerHTML=page==='pullout'?'Workspace <span>/</span> <button type="button" class="breadcrumb-link" id="breadcrumb-clients">Client directory</button> <span>/</span> <b id="crumb">PULLOUT</b>':`Workspace <span>/</span> <b id="crumb">${{dashboard:'Overview',clients:'Client directory',tracker:'Compliance tracker',deadlines:'Deadline reference',settings:'Settings'}[page]}</b>`;breadcrumb.querySelector('#breadcrumb-clients')?.addEventListener('click',()=>go('clients'));document.querySelector('#content').innerHTML=({dashboard:dashboard,clients:clients,pullout:pulloutPage,tracker:tracker,deadlines:deadlines,settings:settings}[page])();document.querySelector('#year')?.addEventListener('change',e=>{const next=Number(e.target.value);if(!e.target.validity.valid||!Number.isInteger(next)){e.target.value=year;return;}year=next;trackerPage=1;render()});bind()}
...
function editClient(id){
  ...
  m.innerHTML=`<h2>${id?'Edit client':'Add a client'}</h2><p>Filing requirements for <strong>${selectedYear}</strong>. Other years keep their existing requirements.</p>
    ...
    <div id="client-registration-fields">
