...
let state;try{state=JSON.parse(localStorage.getItem('taxguard-workspace-v1'))}catch{}if(databaseSnapshot)state={clients:databaseSnapshot.clients,filings:databaseSnapshot.filings};if(!state)state={clients:initial,filings:{}};let page='dashboard',year=2026,query='',filter='unfiled',periodFilter='all';let trackerPage=1;const pageSize=10;let today=localDate();
