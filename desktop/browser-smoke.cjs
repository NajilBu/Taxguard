...
app.whenReady().then(async()=>{
  ...
  const result=await win.webContents.executeJavaScript(`({connected:!!window.taxguardDB,clients:state.clients.length,filings:Object.keys(state.filings).length,footer:document.querySelector('footer span').textContent,settings:(go('settings'),document.querySelector('#content').textContent.includes('Data export & import')&&!!document.querySelector('#open-data-export')&&!!document.querySelector('#open-data-import'))})`);
